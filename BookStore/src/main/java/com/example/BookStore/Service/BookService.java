...
public class BookService {
    ...
    public Book update(int id, Book bookToUpdate)
    {
        var findBook = listBook.stream().filter(p->p.getId() == id).findFirst().orElseThrow();
        if(findBook == null)
            throw new ResponseStatusException(HttpStatus.NOT_FOUND);
        findBook.setAuthor(bookToUpdate.getAuthor());
        findBook.setTitle(bookToUpdate.getTitle());
        findBook.setPrice(bookToUpdate.getPrice());
